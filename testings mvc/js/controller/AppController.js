...
export class AppController {
  ...
  setupRoutingLogic() {
    ...
    if (closeCook)
      closeCook.onclick = () => this.view.toggleCookingModal(false);
  ...
  async handleFavToggle(title, btnElement) {
    ...
    try {
      await this.dataModel.toggleFavorite(
        this.authModel.currentUser.uid,
        title,
        !isFav,
      );
  ...
  setupAuthListeners() {
    ...
    closeBtns.forEach(
      (b) => (b.onclick = () => this.view.toggleAuthModal(false)),
    );
    ...
    if (loginBtn)
      loginBtn.onclick = async () => {
        ...
        const recaptchaResponse = document.querySelector(
          "#loginForm .g-recaptcha-response",
        )?.value;
    ...
    if (regBtn)
      regBtn.onclick = async () => {
        // ПРОВЕРКА ЗА РОБОТ
        const recaptchaResponse = document.querySelector(
          "#registerForm .g-recaptcha-response",
        )?.value;
        if (!recaptchaResponse) {
          this.view.showToast("Моля, потвърдете, че не сте робот!", "error");
          this.view.shakeModal();
